...
class IssueRepository extends BaseRepository {
    ...
    public function create(Issue $issue) {
        ...
        $stmt->bindValue(':pracownik_id', $issue->getIdPracownik());
        $stmt->bindValue(':user_id', $issue->getUserId());
        $stmt->bindValue(':data_wydania', $issue->getDataWydania()->format('Y-m-d H:i:s'));
        try {
            $stmt->execute();
