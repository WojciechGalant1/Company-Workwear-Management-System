...
class OrderHistoryRepository extends BaseRepository {
    ...
    public function create(OrderHistory $orderHistory) {
        ...
        $data_zamowienia = $orderHistory->getDataZamowienia()->format('Y-m-d H:i:s');
        $stmt->bindValue(':data_zamowienia', $data_zamowienia);
        $stmt->bindValue(':user_id', $orderHistory->getUserId());
        $stmt->bindValue(':uwagi', $orderHistory->getUwagi());
        $stmt->bindValue(':status', $orderHistory->getStatus());
        return $stmt->execute();
    }
