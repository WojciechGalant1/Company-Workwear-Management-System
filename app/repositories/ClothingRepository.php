...
class ClothingRepository extends BaseRepository {
    ...
    public function create(Clothing $clothing) {
        $stmt = $this->pdo->prepare("INSERT INTO ubranie (nazwa_ubrania) VALUES (:nazwa_ubrania)");
        $nazwa_ubrania = $clothing->getNazwaUbrania();
    ...
    public function findByName($nazwa) {
        ...
        if ($result) {
            $clothing = new Clothing($result['nazwa_ubrania']);
            $clothing->setIdUbranie($result['id_ubranie']);
            return $clothing;
        }
        return null;
    }
